...
//         className='bg-indigo-600 hover:bg-indigo-700 text-white mr-2' // Add margin-right for spacing
//     >
...
import { Input } from './ui/input';
import { Button } from './ui/button';
import { MessageCircleCode, Send, Upload, FileText } from 'lucide-react'; // Added FileText icon for summarize
import Messages from './Messages';
import axios from 'axios';
import { setMessages } from '@/redux/chatSlice';
...
const ChatPage = () => {
    ...
    const [summary, setSummary] = useState(''); // To store the chat summary
    const { user, suggestedUsers, selectedUser } = useSelector(store => store.auth);
    const { onlineUsers, messages } = useSelector(store => store.chat);
    const { socket } = useSelector(store => store.socketio);
    const dispatch = useDispatch();
        ...
        <div className='flex ml-[20%] h-screen bg-gradient-to-br from-purple-50 to-indigo-50'>
            ...
            {selectedUser ? (
                <section className='flex-1 bg-white rounded-r-2xl shadow-md flex flex-col h-full'>
                    ...
                    {summary && (
                        <div className='bg-indigo-50 p-4 mx-4 my-2 rounded-lg relative'>
                            <h4 className='font-medium text-indigo-700 mb-2'>Conversation Summary</h4>
                    ...
                    <div className='flex items-center p-4 border-t border-gray-200'>
                        ...
                        <Button
                            onClick={() => sendMessageHandler(selectedUser?._id)}
                            className='bg-indigo-600 hover:bg-indigo-700 text-white'
                            disabled={loading || (!textMessage.trim() && !file)} 
                        >
                            {loading ? (
                                <div className="animate-spin rounded-full h-4 w-4 border-t-2 border-white mr-2"></div>
                            ) : (
